...
void ItemManager::loadAll() {
	...
	if (items.empty()) logInfo("No items were found.\n");
	if (item_sets.empty()) logInfo("No item sets were found.\n");
}
...
void ItemManager::loadItems() {
	...
	while (infile.next()) {
		...
		if (id < 1) {
			if (id_line) logError("Item index out of bounds 1-%d, skipping\n", INT_MAX);
			continue;
		}
		if (id_line) continue;
		...
		if (infile.key == "name")
			...
			else logError("Power index inside item %d definition out of bounds 1-%d, skipping item\n", id, INT_MAX);
		}
...
void ItemManager::loadSets() {
	...
	while (infile.next()) {
		...
		if (id < 1) {
			if (id_line) logError("Item set index out of bounds 1-%d, skipping\n", INT_MAX);
			continue;
		}
		if (id_line) continue;
		...
		if (infile.key == "name") {
			...
			while (item_id != "") {
				...
				if (temp_id > 0 && temp_id < static_cast<int>(items.size())) {
					...
					logError("Item index inside item set %s definition out of bounds 1-%d, skipping item\n", cname, maxsize);
				}
