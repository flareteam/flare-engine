...
void ItemStorage::setItems(string s) {
	...
	for (int i=0; i<slot_number; i++) {
		...
		if (storage[i].item < 0) {
			logError("Item on position %d has negative id, skipping\n", i);
			storage[i].item = 0;
			storage[i].quantity = 0;
		}
		else if ((unsigned)storage[i].item > items->items.size()-1) {
			logError("Item id (%d) out of bounds 1-%d, marking as unknown\n", storage[i].item, (int)items->items.size());
			items->addUnknownItem(storage[i].item);
		}
		else if (storage[i].item != 0 && items->items[storage[i].item].name == "") {
			logError("Item with id=%d. found on position %d does not exist, marking as unknown\n", storage[i].item, i);
			items->addUnknownItem(storage[i].item);
		}
	}
}
...
void ItemStorage::setQuantities(string s) {
	...
	for (int i=0; i<slot_number; i++) {
		...
		if (storage[i].quantity < 0) {
			logError("Items quantity on position %d is negative, setting to zero\n", i);
			storage[i].quantity = 0;
		}
	}
}
...
void ItemStorage::clean() {
	for (int i=0; i<slot_number; i++) {
		if (storage[i].item > 0 && storage[i].quantity < 1) {
			logInfo("Removing item with id %d, which has a quantity of 0\n",storage[i].item);
			storage[i].item = 0;
			storage[i].quantity = 0;
		}
	}
