...
void GameStatePlay::saveGame() {
	...
	if (outfile.is_open()) {
		...
		if (outfile.bad()) logError("Unable to save the game. No write access or disk is full!\n");
		outfile.close();
		outfile.clear();
	}
	...
	if (outfile.is_open()) {
		...
		if (outfile.bad()) logError("Unable to save stash. No write access or disk is full!\n");
		outfile.close();
		outfile.clear();
	}
...
void GameStatePlay::loadGame() {
		...
		while (infile.next()) {
			if (infile.key == "name") pc->stats.name = infile.val;
				...
				if (pc->stats.physical_character < 0 || pc->stats.physical_character > pc->stats.max_points_per_stat ||
					...
					logError("Some basic stats are out of bounds, setting to zero\n");
				...
				if (fileExists(mods->locate(mapr->teleport_mapname))) {
					...
					logError("Unable to find %s, loading maps/spawn.txt\n", mapr->teleport_mapname.c_str());
				...
				for (int i=0; i<12; i++) {
					...
					if (hotkeys[i] < 0) {
						logError("Hotkey power on position %d has negative id, skipping\n", i);
						hotkeys[i] = 0;
					}
					else if ((unsigned)hotkeys[i] > powers->powers.size()-1) {
						logError("Hotkey power id (%d) out of bounds 1-%d, skipping\n", hotkeys[i], (int)powers->powers.size());
						hotkeys[i] = 0;
					}
					else if (hotkeys[i] != 0 && powers->powers[hotkeys[i]].name == "") {
						logError("Hotkey power with id=%d, found on position %d does not exist, skipping\n", hotkeys[i], i);
						hotkeys[i] = 0;
					}
				}
	...
	else logError("Unable to open %s!\n", ss.str().c_str());
