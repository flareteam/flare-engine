...
GameStateLoad::GameStateLoad() : GameState()
	...
	if (!found_layer) logError("Could not find layers for direction 6\n");
...
void GameStateLoad::loadPreview(int slot) {
	...
	for (unsigned int i=0; i<equipped[slot].size(); i++) {
		if ((unsigned)equipped[slot][i] > items->items.size()-1) {
			logError("Item in save slot %d with id=%d is out of bounds 1-%d. Your savegame is broken or you might be using an incompatible savegame/mod\n", slot+1, equipped[slot][i], (int)items->items.size()-1);
			continue;
		}
	...
	if (!confirm->visible) {
		...
		if (confirm->confirmClicked) {
			...
			if (remove(filename.str().c_str()) != 0)
				logError("Error deleting save from path");
