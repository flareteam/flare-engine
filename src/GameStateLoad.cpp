	...
	if (infile.open("menus/gameload.txt")) {
		while (infile.next()) {
			...
			if (infile.key == "action_button") {
				...
				portrait_dest.x = eatFirstInt(infile.val, ',');
				portrait_dest.x += (VIEW_W - FRAME_W)/2;
				portrait_dest.y = eatFirstInt(infile.val, ',');
				portrait_dest.y += (VIEW_H - FRAME_H)/2;
				portrait_dest.w = eatFirstInt(infile.val, ',');
				portrait_dest.h = eatFirstInt(infile.val, ',');
			}
