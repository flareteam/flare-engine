	...
	if (infile.open("menus/actionbar.txt")) {
		while (infile.next()) {
			...
			if (infile.key == "slot1") {
				...
				menus[MENU_INVENTORY]->pos.x = eatFirstInt(infile.val, ',');
				menus[MENU_INVENTORY]->pos.y = eatFirstInt(infile.val, ',');
