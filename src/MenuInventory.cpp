...
void MenuInventory::applyItemStats(ItemStack *equipped) {
	...
	for (int i=0; i<MAX_EQUIPPED; i++) {
		...
		while (bonus_counter < item.bonus_stat.size() && item.bonus_stat[bonus_counter] != "") {
			...
			if (id > 0)
				stats->effects.addEffect(id, powers->powers[id].icon, 0, item.bonus_val[bonus_counter], powers->powers[id].effect_type, powers->powers[id].animation_name, powers->powers[id].effect_additive, true, -1, powers->powers[id].effect_render_above,SOURCE_TYPE_HERO);
...
void MenuInventory::applyItemSetBonuses(ItemStack *equipped) {
	...
	for (unsigned k=0; k<set.size(); k++) {
		...
		for (bonus_counter=0; bonus_counter<temp_set.bonus.size(); bonus_counter++) {
			...
			if (id > 0)
				stats->effects.addEffect(id, powers->powers[id].icon, 0, temp_set.bonus[bonus_counter].bonus_val, powers->powers[id].effect_type, powers->powers[id].animation_name, powers->powers[id].effect_additive, true, -1, powers->powers[id].effect_render_above,SOURCE_TYPE_HERO);
		}
	}
}
